...
class ViT(nn.Sequential):
    def __init__(self, feature_forward_expansion=1, channel_forward_expansion=1, emb_size=12, gru_size=12, n_layers=1, timeSeq_forward_expansion=8, n_classes=2, **kwargs):  # depth网络重复结构的深度
        super().__init__()
...
class Trans():
    ...
    def train(self, data_train, label_train,  data_val, label_val, data_test, label_test, test_result_acc, test_result_F1_score):
        ...
        for e in range(self.n_epochs):
            ...
            #     best_test_acc = acc
            #     保存模型
                # torch.save(self.model.state_dict(), 'nBack_class2_LiuyiDevide{}.pth'.format(ith))  # 保存第ith次留一中测试精度最好的模型
    ...
    #  测试
    def test(self, data_train_all, data_test_all, label_test_all):
        self.model.load_state_dict(torch.load('STEW_class2_sub_independent.pth'))
        # z-score
        data_train = data_train_all.reshape(-1,9)
        data_test = data_test_all.reshape(-1,9)
        scaler = StandardScaler()
        train_scaler = scaler.fit_transform(data_train)
        test_scaler = scaler.transform(data_test)
        data_test = np.array(test_scaler).reshape(-1, 3, 5, 14, 9)  # (280,3,4,14,9)
        # 4、加载器#
        data_test = torch.from_numpy(data_test).cuda()
        ...
        with torch.no_grad():
            ...
            Tok2, tmp_label2 = self.model(data_test)
            loss_test_lastone = self.criterion_cls(tmp_label1, tmp_label2, label_test).item()
            y_pred1 = torch.max(tmp_label1, 1)[1]
            y_pred2 = torch.max(tmp_label2, 1)[1]
            acc_test_lastone1 = float((y_pred1 == label_test).cpu().numpy().sum()) / float(
                label_test.size(0))
            acc_test_lastone2 = float((y_pred2 == label_test).cpu().numpy().sum()) / float(
                label_test.size(0))
            acc_test_lastone = (acc_test_lastone1 + acc_test_lastone2)/2
            F1_score = f1_score(np.array(label_test.cpu()), np.array(y_pred2.cpu()))
            print("#" * 30, " 最终在测试集上的测试结果： ", "#" * 30, "\n", "loss:%.4f" % (loss_test_lastone),
                  "acc:%.4f" % (acc_test_lastone), "F1-score:%.4f" % F1_score)

        return acc_test_lastone, loss_test_lastone
...
def main():
    ...
    # acc_KL = []
    # f1_KL = []
    # for KL_we in [0.6,0.5,0.4,0.3,0.2]:
    ...
    for trainIndex, testIndex in logo.split(data_all, label_all, groups=groups):
        ...
        io.savemat('./SCI_STEW_result_data/s{}_weight_spectral.mat'.format(ith), {'weight': weight_spectral})
        weight_temporal = np.array(weight_temporal.cpu())
        io.savemat('./SCI_STEW_result_data/s{}_weight_temporal.mat'.format(ith), {'weight': weight_temporal})
        weight_channel_before = np.array(weight_channel_before.cpu())
        io.savemat('./SCI_STEW_result_data/s{}_weight_channel_before.mat'.format(ith),
                   {'weight': weight_channel_before})
        weight_channel_after = np.array(weight_channel_after.cpu())
        io.savemat('./SCI_STEW_result_data/s{}_weight_channel_after.mat'.format(ith),
                   {'weight': weight_channel_after})
        weight_timeInstance_before = np.array(weight_timeInstance_before.cpu())
        io.savemat('./SCI_STEW_result_data/s{}_weight_timeInstance_before.mat'.format(ith),
                   {'weight': weight_timeInstance_before})
        weight_timeInstance_after = np.array(weight_timeInstance_after.cpu())
        io.savemat('./SCI_STEW_result_data/s{}_weight_timeInstance_after.mat'.format(ith),
    ...
    time_end = time.time()
    torch.save(trans.model.state_dict(), 'STEW_class2_sub_independent.pth')

    # 保存每个被试的测试结果数据：ACC、F1、pred_label、real_label
    io.savemat('./SCI_STEW_result_data/acc_allSub.mat', {'acc': np.array(test_result_acc)})
    io.savemat('./SCI_STEW_result_data/f1_allSub.mat', {'f1': np.array(test_result_F1_score)})
    io.savemat('./SCI_STEW_result_data/label_pred_allSub.mat', {'label': np.array(pred_allSub)})
    io.savemat('./SCI_STEW_result_data/label_real_allSub.mat', {'label': np.array(label_allSub)})
